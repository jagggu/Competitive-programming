...
Input Format
The first argument of input contains a pointer to the head of the linked list A.  The second argument of input contains a pointer to the head of the linked list B.  
...
ListNode* processKNodes(ListNode *head, int k) {
	ListNode *curr = head;
	int cnt = 0;
...
ListNode* Solution::getIntersectionNode(ListNode* A, ListNode* B) {
	...
	int k = abs(ALen-BLen);
	if(ALen > BLen) {
		h1 = processKNodes(A,k);
		h2 = B;
	} else {
		h1 = processKNodes(B,k);
		h2 = A;
	}
